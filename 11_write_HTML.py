# Write table of data to html file and include images.
...
# parse data and write html lines
...
for sim_dir in dir_list:
    if sim_dir.find('lr') == 0: # found a sim run
        ...
        out_line = '%s/%s,%s,%s,%s' % (sim, sim_dir, pTotal,
                                             topAirVelocity,topAirTemperature)
        html_line = '<tr><td>%s/%s</td><td>%.1f</td><td>%.1f</td><td>%.1f</td>' % \
                    (sim, sim_dir, float(pTotal), float(topAirVelocity), float(topAirTemperature))
        for element in element_tags:
            out_line = out_line + ',' + xml_vals[element]
            html_line = html_line + '<td>%.2f</td>' % float(xml_vals[element].encode('ascii'))
        if DEBUG: print out_line + '\n',
        out_text += out_line + '\n'
        out_html += html_line + '</tr>\n'
...
# open html file and write out header and data
outFile = open(os.path.join(base_dir, 'results.html'), 'w')
outFile.write('<html>\n<head></head>\n<body>\n')
outFile.write('<table border="1"></tr>')
outFile.write('<th>sim</th><th>pTotal</th><th>topAirVeleocity</th><th>topAirTemperature</th>')
for element in element_tags:
    outFile.write('<th>%s</th>' % element)
outFile.write('</tr>\n\n')
outFile.write(out_html)
outFile.write('</table>\n')
outFile.write('<a href="outfile.txt">Data File</a><br>\n')
outFile.write('<img src="EvaporativePower.png">\n')
outFile.write('</body>\n</html>')
outFile.close()
